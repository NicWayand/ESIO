...
if [ -z "$CANSIPS_3_DATA_DIR" ]; then
	# Try to source path file
	echo "trying to source path_file.sh"
	source ../path_file.sh
	# Check if its now set
	if [ -z "$CANSIPS_3_DATA_DIR" ]; then
		echo "Need to set CANSIPS_3_DATA_DIR"
		exit 1
	fi
fi
echo $CANSIPS_3_DATA_DIR

mkdir -p $CANSIPS_3_DATA_DIR

# Download
cd $CANSIPS_3_DATA_DIR
wget -nH --cut-dirs=20 -r -A "*CanCM3*" -N $data_ftp
